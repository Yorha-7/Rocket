...
// A component's own axial extent, for placing its CG at the center of that
// extent rather than right on its reference edge. Packed items (parachute,
// shock cord, wadding) report <packedlength>; solid parts report <length>.
...
// Mass of a component whose material is <material type="bulk" density="...">
// applied as a thin shell/annulus (shell_area or annulus already folded into
// the caller's volume), a flat surface (<material type="surface">, e.g. the
// parachute canopy), or a line (<material type="line">, e.g. shock cord).
...
// Resolves a component's CG position (cm from the nose tip) from its
// <position type="top|bottom|middle"> relative to its immediate parent.
// Validated against this design: "top" measures forward-to-aft from the
// parent's start; "bottom" measures from the parent's end (offset can be
// negative, moving the component forward, into the parent); "middle"
// centers on the parent's own midpoint.
...
std::vector<MassComponent> parseOrkMassComponents(const std::string& xml) {
    ...
    // Everything below lives one level deeper, under bodytube's own
    // <subcomponents>, not as a direct child of <bodytube>.
    ...
    // ---- Fin set: flat trapezoidal plates, treated as a rectangle when root==tip ----
    ...
    // ---- Centering rings: annulus, "auto" radii resolve against the body
    //      tube's inner radius and the motor mount tube's outer radius ----
    ...
    for (const XMLElement* ring = body_sub->FirstChildElement("centeringring"); ring;
        ...
        // "auto" (non-numeric) falls back to the surrounding tubes' radii.
